...
export class ReadingsController {
  ...
  public async getByDate(req: Request, res: Response): Promise<void> {
    try {
      const { date, tradition = 'rcl' } = req.query;
      ...
      const traditionId = tradition as string;
  ...
  public async getByDateRange(req: Request, res: Response): Promise<void> {
    try {
      const { start, end, tradition = 'rcl', page = '1', limit = '10' } = req.query;
